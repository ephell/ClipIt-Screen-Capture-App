...
class RightHandle(QGraphicsRectItem):
    ...
    def __init__(self, parent):
        ...
        self.initial_x = self.parent.x() + self.parent.rect().width() - self.rect().width()
    ...
    def update_position(self):
        self.setPos(
            self.parent.x() + self.parent.rect().width() - self.rect().width(), 
            self.parent.y()
        )

    def calculate_and_update_end_time(self):
        handle_top_right_x = self.pos().x() + self.rect().width()
        end_time = self.parent.calculate_time_from_x_pos(handle_top_right_x)
