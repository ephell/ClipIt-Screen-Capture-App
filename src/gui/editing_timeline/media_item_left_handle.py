...
class LeftHandle(QGraphicsRectItem):
    ...
    def __init__(self, parent):
        ...
        self.initial_x = self.parent.x() - self.rect().width()
    ...
    def mouseMoveEvent(self, event):
        if self.drag_start_pos is not None:
            ...
            new_parent_pos_x = int(self.parent.scenePos().x() + delta)
            ...
            min_parent_top_left_x = self.parent.initial_x
            if (
                new_parent_rect_width > 0
                and available_width >= 0 # Prevent handle overlap
                and new_parent_pos_x >= min_parent_top_left_x # Prevent handle out of bounds
            ):
    ...
    def update_position(self):
        self.setPos(
            self.parent.scenePos().x() - self.rect().width(), 
            self.parent.scenePos().y()
        )

    def calculate_and_update_start_time(self):
        parent_top_left_x = self.parent.scenePos().x()
        start_time = self.parent.calculate_time_from_x_pos(parent_top_left_x)
